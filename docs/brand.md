...
The featured Mac wallpaper is the selected 5120 × 2880 Muted gray design, with no rules, a white lockup at bottom left, and the acid SOFTWARE LAB flag at bottom right. Its master is `wallpapers/svg/macos-muted.svg`, with a matching PNG. Both kits retain the original paper/ink Mac assets as well.

The X paper/ink banners use the desktop-and-mobile layout approved on September 21, 2026. Their artwork starts farther right to clear the desktop avatar, while the logo, badge, and headline remain below the mobile status bar and clear of its app controls. Both individual SVG/PNG downloads and both ZIP kits carry these masters.
