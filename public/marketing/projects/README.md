...
- `keyboard.webp`: https://keyboard.experimental.software/ — layout choices, typing passage, and on-screen keyboard, captured September 16.
- `keyboard-clean.webp`: the same capture with the rightmost 16 pixels (browser scrollbar) and bottom 9 pixels cropped, then resized to 1280×720. Used in the current Gallery.
...
- `flights-currency-refresh.webp`: refreshed promotional illustration generated September 16, cropped to 16:9 for the Gallery. The USD boarding-pass motif describes the current USD-only behavior; it is not an application screenshot. Source, Chrome Web Store exports, and generation notes are in `../extension-artwork/google-flights-currency/`. A user-selectable currency remains separate extension work.
